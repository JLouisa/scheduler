...
use crate::data::DbId;
use crate::domain::availability::{field, AvailabilityError, AvailabilitySpot};
...
impl TryFrom<PlanJson> for AvailabilitySpot {
    type Error = AvailabilityError;
    fn try_from(plan: PlanJson) -> Result<AvailabilitySpot, Self::Error> {
...
pub fn get_mock_data(num: &str) -> Result<AvailabilitySpot, AvailabilityError> {
    ...
    let plan = AvailabilitySpot::try_from(plan_json);

    return plan;
}

pub fn get_all_mock_data(num: u16) -> Vec<AvailabilitySpot> {
    let mut availability_list: Vec<AvailabilitySpot> = Vec::new();
    for i in 1..=num {
